...
## Localized GitHub Pages viewer

This fork adds a static viewer at [`curriculum/`](curriculum/) for GitHub Pages. English remains the default source of truth, with language switching for:
...
| Language | Topics | Dependencies |
|---|---|---|
| English | [`data/topics.json`](data/topics.json) | [`data/dependencies.json`](data/dependencies.json) |
| Chinese | [`data/topics_zh.json`](data/topics_zh.json) | [`data/dependencies_zh.json`](data/dependencies_zh.json) |
| Thai | [`data/topics_th.json`](data/topics_th.json) | [`data/dependencies_th.json`](data/dependencies_th.json) |

Lao (`lo`) is not generated in the automatic fallback workflow because there is no stable free local translation provider configured for it.
...
The automatic fallback workflow keeps the JSON structure intact, preserves `{{name}}`, translates only exported text fields, and refuses to publish fake/mock translations.

Export localization units:

```bash
npm run localization:export
```

Run automatic local-provider translation:

```bash
npm run translation:auto
```

If no provider is available, the script stops and writes a blocked report. To try installing a project-local LibreTranslate provider in `.venv`:

```bash
npm run translation:auto:install
```

Then import, validate, and package:
...
Successful packaging creates `os-taxonomy-localized-zh-th-release.zip`. The report explicitly states that this is an automatic machine translation fallback, not native human translation; manual spot-checking is recommended, especially for Thai.

### GitHub Pages deployment

Create a new GitHub repository, push this project, then enable GitHub Pages from the repository settings. If Pages serves the repository root, the curriculum viewer will be available at:

```text
https://<your-user>.github.io/<repo>/curriculum/
```
